...
# ------ set outputdirectory ------
OUTPUTDIR="/data/Projects/linear_ensembles/interp_ensembles/results/imagenetc/logits"
mkdir -p $OUTPUTDIR
...
from_pl=1 # set t0 0 for deepens5
...
store_logits_fname="$OUTPUTDIR/${model}--${dataset_name}--${deepensn}"
echo "Output will be stored in ${store_logits_fname}"
resume="${ENSEMBLE_BASEDIR}/${ENSEMBLEDIR1[$deepensn]}"
echo "$resume"
python /data/Projects/linear_ensembles/interp_ensembles/scripts/eval_imagenet_c_pl.py ${dataset_dir} --arch=${model} \
  --workers=${workers} \
  --resume=${resume} \
  --evaluate \
