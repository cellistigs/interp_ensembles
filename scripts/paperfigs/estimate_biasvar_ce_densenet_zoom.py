...
@hydra.main(config_path = "../script_configs/biasvar/cifar10",config_name = "cifar10_dkl_densenet")
def main(args):
    ...
    if args.get("colormap",False) is True: ## plot assuming scatters are ordered
        colors = np.concatenate([cm.coolwarm(np.linspace(0, 0.5, 6)[:-1]) ,
            cm.coolwarm(np.linspace(0.5,1,15)[1:])])
        ax.scatter(biasvar_array[:,1],biasvar_array[:,0],s=90,c=
                colors,edgecolors="black",linewidths=0.5)
    else:    
        ax.scatter(biasvar_array[:,1],biasvar_array[:,0],s=90,edgecolors="black",linewidths=0.5)
    for i in range(15):    
        ax.plot(np.linspace(0,args.line_extent,100),0.2625+0.0125*i+np.linspace(0,args.line_extent,100),color =
                "black",alpha = 0.2)
    ax.scatter(base_biasvar[1],base_biasvar[0],marker = "x",color = "black",s=90,label = "$\gamma=1$")
    ax.set_title("CIFAR 10: DenseNet121 $\gamma$ models")
    #ax.set_yscale('log')
    #ax.set_xscale('log')
