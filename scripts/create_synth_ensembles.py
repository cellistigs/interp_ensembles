...
def main(args):
    ...
    for si,s in enumerate(subsets):
        ...
        ood_ensemble_results = [get_files(os.path.join(resultspath,dataset_stubs[i]+"ood_preds.npy")) for i in s]
        ...
        np.save(os.path.join(resultspath,"synth_ensemble_{}_{}_ood_preds.npy".format(si,args.nameprefix)),ood_mean)
        np.save(os.path.join(resultspath,"synth_ensemble_{}_{}_ind_labels.npy".format(si,args.nameprefix)),ind_targets)
        np.save(os.path.join(resultspath,"synth_ensemble_{}_{}_ood_labels.npy".format(si,args.nameprefix)),ood_targets)
