...
def make_movie(to_plot,colormap):
    """
    # plot samples
    ...
    for i,(w,d) in enumerate(sort_to_plot.items()):  # 50 frames
        ## calculate angles 
        diff_set = [d["all"][0]-d_prev0,d["all"][1]-d_prev1]
        d_prev0,d_prev1 = d["all"][0],d["all"][1]
        rho,phi = cart2pol(diff_set[1],diff_set[0])
        rbins = np.linspace(0,np.max(rho),30)
        abins_2d = np.linspace(-np.pi+phi_offset,np.pi+phi_offset,30)
        ...
        A,R = np.meshgrid(abins_2d,rbins)
    ...
    subprocess.call("ffmpeg -framerate 5 -pattern_type glob -i {}/'_tmp*.png' \
  -c:v libx264 -pix_fmt yuv420p {}/base_movie.mp4".format(movie_dir,movie_dir), shell=True)
